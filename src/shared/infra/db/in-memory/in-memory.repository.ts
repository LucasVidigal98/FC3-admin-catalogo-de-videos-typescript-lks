...
export abstract class InMemoryRepository<E extends Entity, EntityId extends ValueObject>
  ...
  async delete(enetity_id: EntityId): Promise<void> {
    const indexFound = this.items.findIndex((item) => item.entity_id.equals(enetity_id));

    if (indexFound === -1) {
      throw new NotFoundError(enetity_id, this.getEntity());
    }
